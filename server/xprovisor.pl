...
sub create_session {
  ...
  system("
    ...
    < skeleton.conf sed -e 's+/home/\.skeleton+/home/$session+g' > xbox-$id.conf
...
sub get_leader {
  ...
  my $resp = `${pkgs.systemd}/bin/machinectl show xbox-$id -p Leader`;
...
sub netcat {
  ...
  return qw< ${pkgs.utillinux}/bin/nsenter -a -t >, $pid, qw< ${pkgs.netcat}/bin/nc localhost 5901 >;
}
...
sub setup_hot_spares {
  until((() = glob "$ROOT/sessions/.hot-spare-*/*") >= $NUM_HOT_SPARES) {
    ...
    system("${pkgs.coreutils}/bin/rm", "-rf", "/home/.hot-spare-$id") == 0 or die;
...
sub setup_cold_spares {
  until((() = glob "$ROOT/sessions/.cold-spare-*/*") >= $NUM_COLD_SPARES) {
    ...
    system("
      set -e
      ${pkgs.utillinux}/bin/umount /home/.cold-spare-$id || true
      ${pkgs.coreutils}/bin/mkdir -p /home/.cold-spare-$id
      ${pkgs.utillinux}/bin/mount -t tmpfs none /home/.cold-spare-$id
      ${pkgs.coreutils}/bin/mkfifo /home/.cold-spare-$id/.wait
    ") == 0 or die;
...
sub terminate_idle_sessions {
  ...
  for my $machine (glob "$ROOT/sessions/*/*") {
    ...
    my $idletime = `${pkgs.utillinux}/bin/nsenter -a -t $pid /usr/bin/env DISPLAY=:1 XAUTHORITY=/home/guest/.Xauthority ${pkgs.xprintidle-ng}/bin/xprintidle-ng`;
    if(not $idletime =~ /^\d+$/ or $idletime > $MAX_IDLE_TIME) {
      warn "* Session $machine is idle, terminating...\n";
      rename($machine, $machine . "off");
      system("${pkgs.systemd}/bin/machinectl", "shell", "xbox-$id", "${pkgs.systemd}/bin/poweroff");
      mkdir "$ROOT/clean";
      mkdir "$ROOT/clean/$id";
    }
  }
...
sub reset_idletime {
  ...
  return system(qw<
    ${pkgs.utillinux}/bin/nsenter -a -t>, $pid, qw<
    /usr/bin/env DISPLAY=:1 XAUTHORITY=/home/guest/.Xauthority
    ${pkgs.xdotool}/bin/xdotool mousemove 0 0 mousemove restore
  >) == 0;
}
...
sub clean_obsolete_machines {
  for my $id (glob "$ROOT/clean/*") {
    ...
    unless(get_leader($id)) {
      ...
      system("${pkgs.coreutils}/bin/rm", "-rf", "/var/lib/containers/xbox-$id");
      rmdir "$ROOT/clean/$id";
    }
  }
}
...
sub acquire_session {
  ...
  if(-d "/home/$session" and my $cold_spare = (glob "$ROOT/sessions/.cold-spare-*/*")[0]) {
    ...
    if(rename($cold_spare, "$ROOT/sessions/$session/$id")) {
      ...
      system("${pkgs.utillinux}/bin/mount --bind /home/$session /home/.cold-spare-$id") == 0 or die;
      print $fh "now\n" or die;
      close $fh or die;
      return $id;
    } else {
      warn "  Failure, not using a cold apre.\n";
    }
  }
